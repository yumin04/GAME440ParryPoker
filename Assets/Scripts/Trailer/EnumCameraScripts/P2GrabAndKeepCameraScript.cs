...
public class P2GrabAndKeepCameraScript : MonoBehaviour
{
    [Header("Main Camera")]
    ...
    private CardDataSO cardData;
     ...
     {
         // EnumPositionStorage<P2GrabAndKeepCameraPosition>.player1Cards = new []{1};
         // EnumPositionStorage<P2GrabAndKeepCameraPosition>.player2Cards = new []{1};
         trailerObjectInstantiator.AddMultipleCardsToPlayer1(EnumPositionStorage<P2GrabAndKeepCameraPosition>.player1Cards);
         trailerObjectInstantiator.AddMultipleCardsToPlayer2(EnumPositionStorage<P2GrabAndKeepCameraPosition>.player2Cards);
     }

     private IEnumerator PlayCameraSequence()
     {
         foreach (P2GrabAndKeepCameraPosition pos in System.Enum.GetValues(typeof(P2GrabAndKeepCameraPosition)))
         {
            Debug.Log("[DEBUG] Position: " + pos);

            Transform target = EnumPositionStorage<P2GrabAndKeepCameraPosition>.Positions[(int)pos];

            if (target == null) continue;
            switch (pos)
            {
                case P2GrabAndKeepCameraPosition.InitializeSubRoundCard:
                    ...
                    trailerObjectInstantiator.InstantiateSubRoundCard(EnumPositionStorage<P2GrabAndKeepCameraPosition>.roundCards[subRoundNumber]);
                    yield return new WaitForSeconds(2f);
                    break;
                case P2GrabAndKeepCameraPosition.P2Grab:
                    ...
                    break;
                case P2GrabAndKeepCameraPosition.P2CheckCard:
                    yield return StartCoroutine(MoveCamera(target, 0.3f));
                ...
                case P2GrabAndKeepCameraPosition.P2OptionSelection:
                    yield return StartCoroutine(MoveCamera(target, 0.3f));
                    // Option Pops Up
                    ...
                    yield return new WaitForSeconds(0.5f);
                    break;
                 case P2GrabAndKeepCameraPosition.P2KeepCard:
                    // move the mouse toward Attack, and click
                    trailerObjectInstantiator.MoveMousePointerToKeep();
                    yield return new WaitForSeconds(0.5f);
                    break;
            }
